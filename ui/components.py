...
class FileSelector(ttk.Frame):
    ...
    **kwargs
    ...
    def _setup_drag_drop(self) -> None:
        """Setup drag and drop functionality for the file selector."""
        try:
            # Make the entire frame drop-enabled
            self.drop_target_register(DND_FILES)
            self.dnd_bind("<<Drop>>", self._on_drop)
            self.dnd_bind("<<DragEnter>>", self._on_drag_enter)
            self.dnd_bind("<<DragLeave>>", self._on_drag_leave)
        except Exception as e:
            logger.warning("Failed to setup drag & drop: %s", e)

    def _on_drop(self, event) -> None:
        """Handle file drop event."""
        try:
            # Extract file path from drop event
            files = self.winfo_toplevel().splitlist(event.data)
            if files:
                file_path = files[0].strip("{}")  # Remove curly braces on Windows
                # Check if it's a supported file format
                supported_exts = (".docx", ".md", ".markdown")
                if file_path.lower().endswith(supported_exts):
                    self._file_path_var.set(file_path)
                    if self._on_select is not None:
                        self._on_select(file_path)
        except Exception as e:
            logger.warning("Error handling drop: %s", e)

    def _on_drag_enter(self, event) -> None:
        """Visual feedback when dragging over the widget."""
        self.configure(style="Accent.TFrame")
    ...
    def _browse(self) -> None:
        ...
        path = filedialog.askopenfilename(
            ...
            filetypes=[
                ("Supported Files", "*.docx;*.md;*.markdown"),
                ("Word Documents", "*.docx"),
...
class ResultPanel(ttk.LabelFrame):
    ...
    **kwargs
    ...
    def __init__(self, master: Any, **kwargs: Any) -> None:
        ...
        btn_frame.pack(anchor=tk.W, padx=5, pady=(2, 8))
        ...
        if HAS_TTKB:
            ...
            self._open_file_btn = ttkb.Button(
                btn_frame,
                text="Open file",
    ...
    def show_success(self, output_path: str, stats: dict[str, Any]) -> None:
        ...
        self._open_file_btn.configure(state=tk.NORMAL)
    ...
    def show_failure(self, error_message: str) -> None:
        ...
        self._open_file_btn.configure(state=tk.DISABLED)
    ...
    def _open_file(self) -> None:
        """Open the transformed output file with the system default application."""
        if self._output_path is None:
            return

        if not Path(self._output_path).is_file():
            return
