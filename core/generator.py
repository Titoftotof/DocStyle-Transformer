...
class DocumentGenerator:
    ...
    def _render_element(self, doc: Document, element: ContentElement) -> None:
        ...
        match element:
            ...
            case CodeBlock():
                self._render_code_block(doc, element, style)
            case PageBreak():
    ...
    def _render_code_block(
        ...
        """Render a code block with monospace font and styled background.

        Code blocks are rendered as:
        - A paragraph with a light gray background
        - Monospace font (Consolas/Courier)
        - Preserved whitespace and line breaks
        - Optional language label
        """
        # Get styling
        bg_color_hex = style.get("background", "#f5f5f5")
        border_color_hex = style.get("border_color", "#dddddd")
        font_family = style.get("font_family", "Consolas")
        font_size_hp = style.get("font_size", 18)  # 9pt
        font_color_hex = style.get("color", "dark_gray")

        # Add spacing before/after
        spacing_before = style.get("spacing_before", 200)
        spacing_after = style.get("spacing_after", 200)
        padding = style.get("padding", 120)

        # Container paragraph for background (simulated with shading)
        code_para = doc.add_paragraph()
        code_para.paragraph_format.space_before = Twips(spacing_before)
        code_para.paragraph_format.space_after = Twips(spacing_after)
        code_para.paragraph_format.left_indent = Twips(padding)
        code_para.paragraph_format.right_indent = Twips(padding)

        # Add language label if present
        if code_block.language:
            label_para = doc.add_paragraph()
            label_para.paragraph_format.space_before = Twips(0)
            label_para.paragraph_format.space_after = Twips(80)

            label_run = label_para.add_run(f"{code_block.language}")
            label_run.font.size = Pt(14)  # 7pt
            label_run.font.name = "Calibri"
            label_run.font.color.rgb = _hex_to_rgb(self._design.resolve_color("medium_gray"))
        ...
        for i, line in enumerate(lines):
            ...
            line_para.paragraph_format.space_before = Twips(0)
            line_para.paragraph_format.space_after = Twips(0)
            line_para.paragraph_format.left_indent = Twips(padding)
            line_para.paragraph_format.right_indent = Twips(padding)

            # Add line number if enabled
            if code_block.line_numbers:
                num_run = line_para.add_run(f"{i + 1:3d}  ")
                num_run.font.size = Pt(font_size_hp / 2.0)
                num_run.font.name = font_family
                num_run.font.color.rgb = _hex_to_rgb(self._design.resolve_color("light_gray"))

            # Add code line
            code_run = line_para.add_run(line if line else " ")  # Preserve empty lines
            code_run.font.size = Pt(font_size_hp / 2.0)
            code_run.font.name = font_family
            code_run.font.color.rgb = _hex_to_rgb(self._design.resolve_color(font_color_hex))

        logger.debug(
            "Rendered code block: %d line(s), language='%s'",
            len(lines),
            code_block.language,
        )
