...
class StyleMapper:
    ...
    def map_element(self, element: ContentElement) -> dict[str, Any]:
        ...
        match element:
            ...
            case CodeBlock():
                return self._map_code_block(element)
            case PageBreak():
    ...
    def _map_code_block(self, code_block: CodeBlock) -> dict[str, Any]:
        """Map a code block element to style configuration.

        Code blocks use:
        - Monospace font (Consolas or Courier)
        - Light gray background
        - Small font size for code
        - Optional line numbers
        """
        # Try to get from config, fall back to defaults
        code_style = self._ds.get_component("code_block") or {}

        return {
            "type": "code_block",
            "font_family": code_style.get("font_family", "Consolas"),
            "font_size": code_style.get("font_size", 18),  # 9pt
            "color": code_style.get("color", "dark_gray"),
            "background": code_style.get("background", "#f5f5f5"),
            "border_color": code_style.get("border_color", "#dddddd"),
            "spacing_before": code_style.get("spacing_before", 200),
            "spacing_after": code_style.get("spacing_after", 200),
            "padding": code_style.get("padding", 120),
        }
